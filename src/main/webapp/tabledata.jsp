...
<% while(rst.next()) { %>
			<tr>
				<td colspan="2"><a href="javascript:dRecord('deletedata.jsp?db=<%=db%>&table=<%=table%>&field=<%=rsmd.getColumnName(1) %>&val=<%=rst.getString(1) %>')"><img class="icon" src="./images/Delete.gif" alt="Delete Record"/></a></td>           
<% for(int i=1; i<=rsmd.getColumnCount(); i++) { %>
				<td style="width: 300px;text-align: left;background-color:#f5f5f5"><%=rst.getString(i) %></td>
<% } %>
			</tr>
<%
	num_rows++;
